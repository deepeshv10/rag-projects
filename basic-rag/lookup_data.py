...
def main():
    if not os.path.isdir(CHROMA_DB_DIR):
        print("Error: ChromaDB not found. Run load_data.py first to load documents.")
        return

    gemini_client = get_gemini_client()
    print("RAG Lookup Ready (type 'quit' to exit)\n")
    ...
    while True:
        ...
        for i, (meta, dist) in enumerate(zip(metadatas, distances), 1):
            print(f"  [{i}] {meta['source']} (chunk {meta['chunk']}) | distance: {dist:.4f}")
